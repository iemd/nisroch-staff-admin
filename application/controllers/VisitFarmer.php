...
class VisitFarmer extends CI_Controller {
	...
	public function index()
	{
		...
		$data['VisitFarmerDetails'] = $this->DataModel->VisitFarmerDetails($staff_id);
...
public function update($visit_id=null)
{
	...
	$update = $this->DataModel->StaffUpdateVisitfarmer($visit_id, $data);
	if($update){
		$message = $this->session->set_flashdata('message', 'Updated successfully !');
		redirect(base_url('VisitFarmer/'), 'refresh');
	}
}
...
{
	$this->load->model('DataModel');
	$delete = $this->DataModel->StaffDeleteVisitfarmer($visit_id);
	if($delete){
